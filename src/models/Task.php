...
class Task 
{
    ...
    public static function getSlice(string $orderBy = null, int $limit, int $offset) : array
    {
    ...
    public static function get(int $id, \PDO $pdo = null) : Task
    {
        ...
        else self::$slice[(int)$id] = $handle->fetchObject('\models\Task');
   ...
   public function create(array $data, \PDO $pdo = null) : bool
    {
        ...
        $handle = $pdo->prepare("
            INSERT INTO `task` (user_email, status, edited, name, text) 
            VALUES (:user_email, :status, :edited, :name, :text) 
        ");
    ...
    public function update(array $data, \PDO $pdo = null) : bool
    {
        ...
        else $edited = $data['task_text'] === $this->getText() ? 0 : 1;
